<?php
/**
 * API - Upload de Imagem de Perfil do Utilizador
 */
...
try {
    ...
    error_log("Upload Utilizador - FILES: " . print_r($_FILES, true));
    ...
    error_log("Upload Utilizador - Tamanho do ficheiro: " . $file['size'] . " bytes (" . round($file['size']/1024/1024, 2) . "MB)");
    error_log("Upload Utilizador - Limites PHP: upload_max_filesize={$uploadMaxSize}, post_max_size={$postMaxSize}");
    ...
    // Validar tamanho (máx 5MB para fotos de perfil)
    ...
    if ($file['size'] > $maxSize) {
        ...
        echo json_encode([
            'success' => false,
            'message' => "Imagem muito grande ({$fileSizeMB}MB). Máximo 5MB"
        ]);
        exit;
    }
    ...
    // Caminho completo - de /backoffice/api/users/ para /backoffice/uploads/users/
    ...
    $relativePath = '/backoffice/uploads/users/' . $filename;
